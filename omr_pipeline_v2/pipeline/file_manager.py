...
'''def setup_pi_reduction_dirs(hierarchy, flat_pool, all_bias, all_dark,
    ...
    if os.path.exists(pi_folder_path):
        reset_pi = messagebox.askyesno(
            "PI Folder Exists",
            f"The folder for {selected_pi} already exists.\n\n"
            "Click YES to DELETE the entire PI folder and start fresh.\n"
            "Click NO to check individual configuration sub-folders.",
        )
        if reset_pi:
            try:
                shutil.rmtree(pi_folder_path)
                os.makedirs(pi_folder_path)
                log(f"Re-created PI folder: {pi_folder_path}")
            except Exception as e:
                log(f"Error deleting PI folder: {e}")
                return {}
    else:
        os.makedirs(pi_folder_path)
