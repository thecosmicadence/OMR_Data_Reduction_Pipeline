...
from pipeline.file_manager import (
    group_files_by_config,
    setup_reduction_dirs,
...
class Pipeline:
    ...
    def select_and_setup_directory(self):
        ...
        # ── ccdhedit: set DISPAXIS on every raw .fit before segregation ──────
        self._run_ccdhedit_on_raw(original_path)

        groups = group_files_by_config(original_path, self.log)
        if not groups:
            self.log("No FITS frames found or grouped. Aborting.")
            return

        setup_reduction_dirs(groups, original_path, self.gui, self.sets, self.log)

        # Ask user which set to process
        set_choices = [f"GRATING={g}, CENTWAVE={c}" for (g, c) in self.sets]
        choice = self.gui.show_file_dropdown(set_choices, "Select data set to reduce")
        if not choice:
            self.log("No set selected. Exiting.")
            return

        selected_key = None
        for (g, c) in self.sets:
            if f"GRATING={g}, CENTWAVE={c}" == choice[0]:
                selected_key = (g, c)
                break
        ...
        self.reduction_path = self.sets[selected_key]
        os.chdir(self.reduction_path)
        self.log(f"Working directory set to: {self.reduction_path}")

        populate_pipeline_lists(self)

    # ── ccdhedit on raw data ─────────────────────────────────────────────────

    def _run_ccdhedit_on_raw(self, raw_path):
        """Run ``ccdhedit`` to set DISPAXIS=1 on every .fit file in *raw_path*.

        This is executed before data segregation so that the keyword is
        already present when files are copied into per-grating reduction
        directories.
        """
        import glob

        fits_pattern = os.path.join(raw_path, "*.fit")
        fit_files = sorted(glob.glob(fits_pattern))
        if not fit_files:
            self.log("ccdhedit: no .fit files found in raw directory — skipping.")
            return

        self.log(f"\nRunning ccdhedit (DISPAXIS=1) on {len(fit_files)} raw .fit files in:\n  {raw_path}")
        self.init_iraf()
        ...
        try:
            ...
            iraf.ccdhedit()
            self.log("ccdhedit: DISPAXIS keyword updated successfully.")
        except Exception as e:
            self.log(f"ccdhedit: WARNING — could not update DISPAXIS: {e}")
        finally:
            os.chdir(old_dir)
    ...
    def start_apall(self):
        ...
        if not self.comp:
            self.comp = sorted(
                glob.glob("*co*bd.fit") + glob.glob("*co*b.fit")
            )
        if not self.obj:
            self.obj = sorted(glob.glob("*obj*bdf.fit"))
