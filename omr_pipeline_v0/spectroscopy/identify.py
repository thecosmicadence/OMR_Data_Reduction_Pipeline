...
from utils.signal import measure_shift_subpixel


def compute_shifts(ref_flux, extracted_comp_files):
    """
    Compute sub-pixel shifts of all comparison frames relative to a reference flux.
...
def run_identify(pipeline):
    ...
    for frame, shift in shift_dict.items():
        pipeline.log(f"Measured shift vs Master: {frame} -> {shift:.2f} pixels")
    ...
    subprocess.run(
        ["pyraf", "-c",
         f"noao; imred; ccdred; specred; identify(images='{ref_frame}', mode='h')"],
        env=env,
    )
