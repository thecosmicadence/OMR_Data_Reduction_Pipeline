...
from spectroscopy.specshift import apply_wavelength_shift, plot_lamp_spectra
...
class Pipeline:
    ...
    def __init__(self):
        ...
        self.corrected_obj   = None
        self.corrected_comp  = None
        self.trimmed = None
        self.gui = None
    ...
    def start_specshift(self):
        self.init_iraf()
        os.chdir(self.reduction_path)
        apply_wavelength_shift(self)

    def start_lamp_plot(self):
        os.chdir(self.reduction_path)
        plot_lamp_spectra(self)
    ...
    def exit_pipeline(self):
        ...
        msg = (
            ...
            "Yes: Process another directory (Restart pipeline).\n"
            "No: Exit and close the pipeline entirely.\n"
            "Cancel: Stay in the current session."
        )
        choice = messagebox.askyesnocancel("Exit / Restart Pipeline", msg)
        ...
        if choice is True:
            self.log("Restarting pipeline state for a new directory...")
            ...
            if self.gui is not None:
                self.gui.output_area.config(state="normal")
                self.gui.output_area.delete(1.0, tk.END)
                self.gui.output_area.config(state="disabled")

            # Launch directory selection again
            self.select_and_setup_directory()
            return
