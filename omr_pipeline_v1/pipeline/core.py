...
class Pipeline:
    ...
    def select_and_setup_directory(self):
        ...
        if os.path.exists(os.path.join(original_path, "master_bias.fit")):
            try:
                os.remove(os.path.join(original_path, "master_bias.fit"))
        ...
        # Collect unique bias frames across all groups and create master_bias.fit
        # in the raw data directory so it can be copied into each reduction dir.
        
        all_bias = list({fname for g in groups.values() for fname in g["bias"]})
        if all_bias:
            self.init_iraf()
